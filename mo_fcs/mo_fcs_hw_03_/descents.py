...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        batch = np.random.randint(0, y.shape[0], self.batch_size)
        gradient = 2 * x[batch].T @ (x[batch] @ self.w - y[batch]) / self.batch_size
        return gradient
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        weight_diff = (- self.lr.__call__() / (np.sqrt(v_hat) + self.eps)) * m_hat
        self.w += weight_diff
        return weight_diff
